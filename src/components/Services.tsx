...
const servicesData: ServiceItem[] = [
  ...
  },
  {
    ...
    description: "Somatic decompression zones, contrast cold plunge therapy, custom infrared saunas, and active localized recovery protocols designed to accelerate muscular restoration and mental balance."
  },
  {
    id: "workspace",
    number: "03",
...
export default function Services() {
  ...
  return (
    <section id="services" onMouseMove={handleMouseMove} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} className="bg-surface relative overflow-hidden border-t border-white/10">
      ...
      <div className="absolute top-[22%] left-1/2 -translate-x-1/2 -translate-y-1/2 select-none pointer-events-none font-display-xl text-[13vw] uppercase tracking-[0.3em] text-white/[0.012] font-black leading-none whitespace-nowrap z-0">PERFORMANCE</div>

      <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
        <motion.div animate={{ x: [0, 40, -20, 0], y: [0, -30, 20, 0] }} transition={{ duration: 25, repeat: Infinity, ease: "easeInOut" }} className="absolute -top-[10%] -left-[10%] w-[45%] h-[45%] rounded-full bg-primary/4 blur-[130px]" />
        <motion.div animate={{ x: [0, -50, 30, 0], y: [0, 30, -35, 0] }} transition={{ duration: 30, repeat: Infinity, ease: "easeInOut" }} className="absolute -bottom-[10%] -right-[10%] w-[45%] h-[45%] rounded-full bg-primary/3 blur-[130px]" />
      </div>

      <div className="absolute inset-0 pointer-events-none transition-opacity duration-700 z-0" style={{ opacity: isHovered ? 1 : 0.3, background: isHovered ? `radial-gradient(800px circle at ${mousePos.x}px ${mousePos.y}px, rgba(42, 183, 255, 0.08), transparent 80%)` : "radial-gradient(800px circle at 50% 50%, rgba(42, 183, 255, 0.04), transparent 80%)" }} />

      <div className="px-6 md:px-16 pt-24 pb-12 border-b border-white/10 text-center flex flex-col items-center relative z-10">
        <motion.h2 initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} className="font-display-xl text-5xl md:text-7xl uppercase mb-6 text-primary-container">
          <FlipWord word="WHAT WE DO" triggerOnView />
        </motion.h2>
        <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.6, delay: 0.15 }} className="font-body-lg text-on-surface-variant max-w-2xl mx-auto">
            ...
            <motion.div key={service.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.5, delay: index * 0.1 }} onClick={() => navigateToService(service.id)} onMouseEnter={() => setActiveItem(service.id)} onMouseLeave={() => setActiveItem(null)}
              ...
              <div className="flex flex-col gap-3 relative z-10 select-none max-w-[80%]">
                <div className="flex items-center gap-8">
                  <span className="font-label-md text-outline opacity-40 transition-opacity group-hover:opacity-80 min-w-10">{service.number}</span>
                  <h2 className={`service-name font-display-xl text-4xl sm:text-5xl md:text-7xl text-white uppercase transition-all duration-500 transform ${isOpen ? "text-primary translate-x-4 md:translate-x-6" : ""}`}>{service.title}</h2>
                </div>
                <div className={`transition-all duration-500 overflow-hidden transform origin-left ${isOpen ? "max-h-96 opacity-100 translate-y-0 pl-12 sm:pl-16 md:pl-[7.5rem] mt-2" : "max-h-0 opacity-0 -translate-y-2 pl-12 sm:pl-16 md:pl-[7.5rem] pointer-events-none"}`}>
                  <p className="text-gray-200/95 font-sans text-sm sm:text-base md:text-lg max-w-2xl leading-relaxed tracking-wide">{service.description}</p>
                </div>
              </div>
              <div className={`plus-icon w-12 h-12 md:w-16 md:h-16 border rounded-full flex items-center justify-center transition-all duration-500 relative z-10 select-none ${isOpen ? "border-primary bg-primary text-on-primary rotate-45 scale-110 shadow-[0_0_15px_rgba(42,183,255,0.4)]" : "border-white/20 text-white"}`}>
